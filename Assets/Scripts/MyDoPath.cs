    ...
    public void PlanePlacement()
    {
        ...
        for (int i1 = 0; i1 < ItemData.Instance.field.moneyPlane; i1++)
        {
            ...
            for (int i2 = 0; i2 < Ball.BallsGO.Count - 1; i2++)
            {
                ...
                BallTower[i1].BallsV3[i2] = new Vector3(Ball.BallsGO[i2].transform.position.x, Ball.BallsGO[i2].transform.position.y + BuyPlane.Instance.moneyPlaneDistance * i1, Ball.BallsGO[i2].transform.position.z);
                if (i2 == Ball.BallsGO.Count - 2)
                {
                    BallTower[i1].BallsV3[i2 + 1] = new Vector3(Ball.BallsGO[i2 + 1].transform.position.x, Ball.BallsGO[i2 + 1].transform.position.y + BuyPlane.Instance.moneyPlaneDistance * i1, Ball.BallsGO[i2 + 1].transform.position.z);
                    BallTower[i1].BallsV3[i2 + 2] = new Vector3(RunnerManager.Instance._runnerPos.transform.position.x, RunnerManager.Instance._runnerPos.transform.position.y + BuyPlane.Instance.moneyPlaneDistance * i1, RunnerManager.Instance._runnerPos.transform.position.z);
                }
            }
